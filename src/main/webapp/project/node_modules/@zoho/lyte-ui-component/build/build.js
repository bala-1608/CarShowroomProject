...
module.exports = {
	...
	configureFolders: function (options) {
		...
		options.minifyAllFiles = options.production ? true : false;
	...
	build: async function(options, dependencies) {
			...
			for (; i < len; i++) {
				for (key in json) {
					for (var j = 0; j < dirLen; j++) {
						if(json[key][0].indexOf('global.less') == -1) {
							json[key].unshift("styles/global.less");
						}
						options.themeOptions = {
							globalVars: {
								themeName: themes[i],
								stylesWithVariable: false,
		...
		try {
			...
			var directions = ['ltr', 'rtl'],
				dirLen = directions.length, i = 0, key;
			...
			for (key in json) {
				for (var j = 0; j < dirLen; j++) {
					...
					await buildUtils.compileThemes({
						src: json[key],
						dist: 'compiledCSS/css-variables/styles/' + directions[j] + '/' + key
					});
				}
			}
		} catch (e) {
		...
		try {
			data = fs.readFileSync('build/less-css-variables-conf.json', 'utf-8');
			json = JSON.parse(data);
			var themes = ['css-variables', 'css-variables-night'],
				folders = ['default', 'night']
				directions = ['ltr', 'rtl'],
				len = themes.length, dirLen = directions.length, i = 0, key;
			...
			for (; i < len; i++) {
				for (key in json) {
					...
					await buildUtils.compileThemes({
						src: json[key],
						dist: 'compiledCSS/css-variables/variables/' + folders[i] + '/' + key
					});
				}
			}
		} catch (e) {
			...
			for (var j = 0; j < dirLen; j++) {
				for (key in json) {
					...
					options.themeOptions = {
						globalVars: {
							themeName: 'css-variables',
							stylesWithVariable: true,
							direction: directions[j]
						}
					}
					var outputFileName = key.replace("-css-variables", "")
					await buildUtils.compileThemes({
						src: json[key],
						dist: 'compiledCSS/css-variables/' + directions[j] + '/' + outputFileName
					});
				}
			}
		} catch (e) {
		...
		try {
			data = fs.readFileSync('build/shadowdom-less-min-conf.json', 'utf-8');
			json = JSON.parse(data);
			var themes = ['default', 'night'],
				directions = ['ltr', 'rtl'],
				len = themes.length, dirLen = directions.length, i = 0, key;
			...
			for (; i < len; i++) {
				for (key in json) {
					for (var j = 0; j < dirLen; j++) {
						...
						await buildUtils.compileThemes({
							src: json[key],
							dist: 'shadowDomCompiledCSS/' + themes[i] + '/' + directions[j] + '/' + key
						});
					}
				}
			}
		...
		try {
			...
			for (key in json) {
				for (var j = 0; j < dirLen; j++) {
					...
					await buildUtils.compileThemes({
						src: json[key],
						dist: 'shadowDomCompiledCSS/css-variables/styles/' + directions[j] + '/' + key
					});
				}
			}
		} catch (e) {
	...
	builder: {
		...
		copyAppDir: async function (module) {
			...
			var arr = ['I18n'];
			for(let indx = 0,len =  arr.length;indx < len ;indx++) {
				let folder =arr[indx];
				await buildUtils.copy.folder({
					src : path.join(buildOptions.root,folder),
					dist : path.join(buildOptions.outputFolder,folder),
