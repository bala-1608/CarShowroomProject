<template tag-name="lyte-combobox">
	<template lyte-if="{{ltPropLabel}}">
        <div class="lyteComboboxFieldLabel" id="{{lyteUiGetComboboxLabelId()}}">{{ltPropLabel}}</div>
    </template>
			...
			<%if(! ltPropYield ) {%>
				<lyte-drop-button> 
					...
					<% } else { %>
						...
						<% if(selectValue && ltPropShowRemoveIcon) { %>
