...
Lyte.Component.register( 'lyte-emoji', {
	data : function(){
		...
		return {
		  ...
		  /**
		   ...
		   /**
			* @componentProperty { startsWith | endsWith | indexOf } ltPropMethod=indexOf
			* @version 3.86.0
			*/					
			ltPropMethod : Lyte.attr( 'string', { default : default_values.method || "indexOf" }),
		   /**
			* @componentProperty {string} ltPropNoMatchText=No results matched your search
			* @version 3.86.0
			*/				
			ltPropNoMatchText : Lyte.attr( 'string', { default : default_values.noMatchText || "No results matched your search" }),
		   /**
			...
			*/	
			...
			/**
			* @componentProperty {boolean} ltPropShowTitle
			* @default false
			* @version 3.107.0
			*/	
			ltPropShowTitle : Lyte.attr( 'boolean', { default : false } ),
	...
	init : function (){
		this.data.ltPropInput ? null : this.setData( 'ltPropInput' , { type : "search", placeholder : "Search Emoji", closeIcon : true, appearance : "box", aria : true } ) ;
	},
	...
	actions : {
		...
		enter : function( name, _this ){
			_this.setAttribute( "lt-prop-title", _lyteUiUtils.i18n( name.toLowerCase().replace( /\s/g, "-" ), 'emoji', name ) );
		},

		keydown : function( evt ){

			if( !evt.target.classList.contains( 'lyteEmoji' ) || this.data.ltPropInput.tabIndex == -1 ){
				return;
			}

			var focusElement = function( isNxt ){
				var ele = isNxt ? document.activeElement.nextElementSibling : document.activeElement.previousElementSibling;

				if( !ele || ele.classList.contains('lyteEmojiGroupTitle') ){
					ele = document.activeElement.parentElement[isNxt ? 'nextElementSibling' : 'previousElementSibling'];
					
					if( !ele.classList.contains('lyteEmojiGroup') ){
						return;
					}else{
						ele = isNxt ? ele.querySelector( '.lyteEmoji' ) :  ele.children[ ele.children.length - 1 ];
					}
				}

				if( ele && ele.classList.contains( 'lyteEmoji' ) ){
					ele.focus();
					evt.preventDefault();
				}
			}
...
/**
 * @syntax Emoji
 * <lyte-emoji></lyte-emoji>
 */
/* default value of emoji TODO :{{Varun}} discuss with sonu 
