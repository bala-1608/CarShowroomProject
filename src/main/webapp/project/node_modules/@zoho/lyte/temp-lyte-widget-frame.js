...
var widgetScopeMap = new Map(), widgetStyles = new Map(), widgetCompStyles = new Map(), widgetShadows = new Map(), widgetMap = new Map(), prxWkMap = new WeakMap();
...
if(isWidget) {
    ...
    if(window.isLyteWidget) {
        ...
        Object.defineProperty(window, "print", {
            "get" : async function() {
                return $Lt._aW(await window.getDetailsFromParent(window, "print", false, []));
            }
        })
...
;(function(Lyte) {
    ...
    window.execStack1 = function() {
        let currentStack = __stack.shift();
        try {
            if(!(currentStack.data && currentStack.data.p == "2")) {
                __stackStatus = 1;
            }
            eventHandlerChild(currentStack);
        } catch(e) {
            console.error(e);
        }
        __stackStatus = 0;
    ...
    async function eventHandlerChild(event) {
        ...
        if(data.p) {
            ...
            switch(data.p) {
                ...
                break;
                case "1a" : {
                    await loadWidget(data.w, data.src, data.c);
                    window.parent.postMessage({
                        "c" : "1ar", 
                        "re" : param1,
                        "done" : true
                ...
                break;
                case "13" : {
                    let retVal = Lyte.Widget.destroy(param1, true);
                    retVal.then(function() {
                        window.parent.postMessage({"c" : "13r", "r1" : param2}, "*");
                    }, function() {
                        window.parent.postMessage({"c" : "13r", "r1" : param2, "s" : 0}, "*");
                    });
                ...
                case "16" : {
                    ...
                    let options = param4;
                    window.LyteWidgetClass.triggerEvent(eventName, eventData, options, widgetName);
                }
                break;
            }
    ...
    function handleInitProperties(node, dataManipulation) {
        ...
        if(node._initMethods && (initMethods = Object.keys(node._initMethods))) {
            for(let k=0;k<initMethods.length;k++) {
                initMethods[k] = _LC.String.dasherize(initMethods[k]);
            }
        }
        return {"iP" : initProperties, "eC" : elementCount.elmCnt, "iM" : initMethods};
    }
    function loadWidget(name, src, componentName){
        let styleArr
        if(componentName) {
            if(!widgetCompStyles.get(name)) {
                widgetCompStyles.set(name, {});
            }
            let compStyles = widgetCompStyles.get(name);
            styleArr = compStyles[componentName] = [];
        }
        widgetStyles.set(name, []);
        var prmArr = [];
        return _lyteWidgetInit(name, src, styleArr);
        // src.forEach(function(itm){
        //     prmArr.push(Lyte.widgetInit(name, itm));
        // });
        // return Promise.all(prmArr);
    }
    function initStylesToWidget(shadow, widgetName, componentName) {
        let compStyles;
        compStyles = ( componentName && (compStyles = widgetCompStyles.get(widgetName))) ? compStyles[componentName] : [];
        var styles = widgetStyles.get(widgetName) || [];
        styles.push.apply(styles, compStyles);
        var docFrag = document.createDocumentFragment();
        for(var i=0;i<styles.length;i++) {
            if(styles[i].styleTag) {
                    ...
                    mutation.addedNodes.forEach(function(node) {
                        if(!node._m) {
                            if(node.nodeType == 1) {
                                ...
                                obj.addNodes.push({"type" : "element" , "tagName" : node.tagName, "_m" : node._m, "childNodes" : childNodeProcessed, "prev" : node.previousSibling ? node.previousSibling._m : undefined, "attrs" : attrs, "initProperties" : initPropReturn.iP, "initMethods" : initPropReturn.iM, "elementCnt" : initPropReturn.eC});
                                // someArray.push(node);
...
var oldElementEventListenerAdd = HTMLElement.prototype.addEventListener;
ShadowRoot.prototype.addEventListener  = HTMLElement.prototype.addEventListener = document.addEventListener = function() {
    ...
    if(this instanceof ShadowRoot) {
        ...
        if(this instanceof HTMLElement) {
            return oldElementEventListenerAdd.apply(this, arr);
        } else {
            return oldDocumentEventListenerAdd.apply(this, arr);    
        }
    }
...
let oldElementEventListenerRemove = HTMLElement.prototype.removeEventListener;
ShadowRoot.prototype.removeEventListener = HTMLElement.prototype.removeEventListener = document.removeEventListener = function() {
    ...
    if(this instanceof ShadowRoot) {
        ...
        return oldElementEventListenerRemove.apply(this, arr);
    } else {
...
(async function(){
    ...
    const cloneOpts = {
        ...
        "Window" : {
            ...
            "functions" : ["fetch" , "addEventListener", "getSelection","unescape","clearInterval","clearTimeout","alert","prompt","confirm","print", "prop", "getComputedStyle"],
            "copyClone" : false
        }, 
        "Document" : {
            "type" : "Document",
    ...
    function addToWidget(fStr, widgetName, fileName, type, styleArr){
        if(type == "style") {
            ...
            styleArr.push({"name" : fileName, "string" : fStr, "styleTag" : true});
            return;
        }
