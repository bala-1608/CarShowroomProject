...
_template:"<template tag-name=\"lyte-gradient-slider\"> <template is=\"if\" value=\"{{ltPropControls}}\"><template case=\"true\"><div class=\"lyteGradientControls\"> <span class=\"lyteGradientInsert\" __click=\"{{action('insert',event)}}\"></span> <span class=\"lyteGradientRemove\" __click=\"{{action('remove',event)}}\"></span> </div></template></template> <div class=\"lyteGradientFillContainer\" style=\"background-image: {{ltPropBackgroundImage}};\"> <template items=\"{{ltPropStop}}\" item=\"item\" index=\"index\" is=\"for\"><span class=\"lyteGradientHandler\" tabindex=\"{{ltPropTabIndex}}\" data-index=\"{{index}}\" ontouchstart=\"{{action('mousedown',event,item)}}\" aria-valuemin=\"0\" aria-valuemax=\"1\" aria-valuenow=\"{{item.offset}}\" aria-orientation=\"horizontal\" aria-valuetext=\"{{item.color}} {{item.offset}}\" style=\"{{lyteGradientSliderHandlerPosition(item.color,item.offset)}}\" __focus=\"{{action('focus',this)}}\" __mousedown=\"{{action('mousedown',event,item)}}\" __keydown=\"{{action('keydown',event,item)}}\"></span></template> </div> </template>",
