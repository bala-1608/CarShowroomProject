...
_template:"<template class=\"lyteSpeedDialComp\" tag-name=\"lyte-speed-dial\"> <div class=\"lyteSpeedDial\" style=\"--lyte-speed-dial-icon-size : {{ltPropIconSize}}px;\"> <div class=\"lyteSpeedDialIconWrapper lyteSpeedDialButtonIcon\"> <img class=\"lyteSpeedDialMainIcon\" src=\"{{ltPropSpeedButton.src}}\" alt=\"{{ltPropSpeedButton.name}}\" __click=\"{{action('speedIconClick',this)}}\"> </div> <template is=\"if\" value=\"{{ltPropSpeedShow}}\"><template case=\"true\"> <template items=\"{{ltPropIcons}}\" item=\"item\" index=\"index\" is=\"for\"> <div id=\"{{index}}\" class=\"lyteSpeedDialIconWrapper\" lt-prop-title=\"{{if(ifEquals(ltPropTooltipShow,true),item.name,'')}}\" lt-prop-tooltip-config=\"{{ltPropTooltipConfig}}\" style=\"--lyte-speed-dial-icon-index: {{index}}\" __mouseover=\"{{action('onOver',this)}}\" __click=\"{{action('iconClick',this)}}\"> <img src=\"{{item.src}}\" class=\"lyteSpeedDialIcon\" alt=\"{{item.name}}\"> </div> </template> </template></template> </div> </template>",
