...
_template:"<template class=\"lyteDockComp\" tag-name=\"lyte-dock\"> <template is=\"if\" value=\"{{ltPropBindToBody}}\"><template case=\"true\"> <lyte-wormhole lt-prop-query=\"{{ltPropQuery}}\" on-before-append=\"{{method(&quot;beforeWormholeAppend&quot;)}}\"> <template is=\"registerYield\" yield-name=\"lyte-content\"> <div class=\"lyteDock lyteIcons\" __mouseout=\"{{action('onDockOut')}}\"> <template items=\"{{ltPropIcons}}\" item=\"item\" index=\"index\" is=\"for\"> <div id=\"{{index}}\" class=\"lyteDockIconWrapper\" style=\"--lyte-dock-icon-wrapper-size : {{ltPropIconSize}};\" onmouseleave=\"{{action('onOut',event,this)}}\" __mouseover=\"{{action('onOver',event,this)}}\" __click=\"{{action('iconClick',this)}}\"> <template is=\"if\" value=\"{{ltPropTooltipShow}}\"><template case=\"true\"><span class=\"lyteDockTooltip\">{{item.name}}</span></template></template> <img class=\"lyteIcon\" type=\"image\" src=\"{{item.src}}\" alt=\"{{item.name}}\"> </div> </template> </div> </template> </lyte-wormhole> </template></template> </template>",
