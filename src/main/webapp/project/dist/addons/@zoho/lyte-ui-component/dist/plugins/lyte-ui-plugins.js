...
( function( factory ) {
    if( typeof define === "function" && define.amd ) {
		define( [ "@zoho/lyte-dom", "@zoho/lyte-ui-component/components/helpers/helpers-dev.js" ], function( $L, module ){
            factory( $L, module._lyteUiUtils );
        } );
    }
    else {
        factory( $L, _lyteUiUtils );
    }
} )(function( $L, _lyteUiUtils ) {
                    ...
                    if(data && data._isDown){
                            ...
                            if(typeof data.helper == "string"){
                                ...
                                if(helper){
                                    ...
                                    if(data.appendTo == "parent"){
                                        _lyteUiUtils.appendChild(data._parent,helper);
                        ...
                        if(returnVal){
                            ...
                            if(data.hasContainment && returnVal){
                                ...
                                if(elem.getBoundingClientRect().bottom >= (top + height)){
                                    elem.style.top = (top + height) - offsetTop - elem.getBoundingClientRect().height + _borderVal.bottom + _scrollTop + 'px';
                                }
                                ...
                                if(elem.getBoundingClientRect().right >= (parent.getBoundingClientRect().left + width)){
                                    elem.style.left = (parent.getBoundingClientRect().left - offsetLeft + width)   - elem.getBoundingClientRect().width + _borderVal.right + _scrollLeft  + 'px';
                                }
                            }
                        }
...
;( function( cb ){
	if( typeof define == "function" && define.amd ){
		define( [ "@zoho/lyte-dom", "@zoho/lyte-ui-component/components/helpers/helpers-dev.js" ], function( $L, module ){
            cb( $L, module._lyteUiUtils );
        } );
	} else {
		cb( window.$L, _lyteUiUtils );
	}
} )( function( $L, _lyteUiUtils ){
	if( $L ){
...
( function( factory ) {
	...
	if($L){	
		var postpone = (function () {
		    var fnMap = new window.Map(), idMap = new window.Map(), fnId = 0;
		    var msg = { fnId: 0 };
		...
		$L.prototype.mentionsInput = function(object) {
			...
			var checkForMaxWidth = function(lyteMIList){
				if(window.getComputedStyle(lyteMIList).maxWidth === "none"){
					var cs = window.getComputedStyle(lyteMIList._textarea);
	...
	if($L){
		...
		$L.prototype.sortable = function(object) {
			...
			if(element._sortableParentData){
				...
				data.context = data.context ? data.context : 'BODY';
				...
				if(_sortableElem){
					...
					if(data && data._isDown){
						...
						data._maxScrollWidth = _maxScrollWidth;
							...
							//console.log(divOffset.top,_scrollTop);
									...
									if(data.orientation == "vertical"){
										...
										if(leftVal+parentOffset.left >= data.containmentDimensions.left && leftVal+parentOffset.left <= (data.containmentDimensions.left + (data.containmentDimensions.width - divOffset.width) + scrollLeftValue)){
											_div.style.left = leftVal + 'px';
											 ...
											 //console.log("leftVal ===>", (limit.left - parentOffset.left));
								...
								var measure2 = $L.fastdom.measure(function(){
									...
									if(data.multiSortable) {
										...
										if(  droppablePlace && $L(droppablePlace).hasClass('sortable-parent') ){
											...
											if ( droppablePlace && !droppablePlace.parentElement._sortableParentData.droppable){
												droppablePlace = false;
											}
										}
									}
											...
											if($L(_elemBelow).hasClass('sortable-parent') && checkParentDroppable(_elemBelow,_sortableElem.parentElement,_sortableElem,data.connectedWith,data.containmentDimensions,_mousePosition) && checkForIntersect(_elemBelow,_mousePosition) && checkForBetween(_elemBelow,_mousePosition,_sortableElem/*,isRelativeY,scrollDiv*/)){
												...
												if(data.tolerance == "pointer"){
													if(data.orientation === "horizontal"){
														...
														if(((data.gridView && (clientX < data._prevLeft)) || (clientY < data._prevTop)) && (droppablePlace.previousElementSibling != _placeholder /*!_placeholder.nextElementSibling || (_placeholder.nextElementSibling && _placeholder.nextElementSibling != droppablePlace)*/)){	//!placeholder.nextElementSibling to check if placeholder is the last element and the draggable element's top goes above the last element's top
															// console.log("calling for droppable ele insertBefore => ",droppablePlace.textContent);
															_lyteUiUtils.insertBefore(droppablePlace,_placeholder);
															callPlaceholder(data, _elemBelow, droppablePlace);
														}
														else if(((data.gridView && (clientX > data._prevLeft)) || (clientY > data._prevTop)) && (droppablePlace.nextElementSibling != _placeholder /*!_placeholder.previousElementSibling || (_placeholder.previousElementSibling && _placeholder.previousElementSibling != droppablePlace)*/)){
			...
			var callForScrollY = function(data,scrollDiv,parentOffset,_maxScrollHeight,topNBottom,isRelativeY,_mousePosition,_offset,parent){
				...
				$L.fastdom.measure(function(){
					...
					$L.fastdom.mutate(function(){
						...
						if( isElementReachedTop( data,divOffset,scrollDivTop) && (scrollTop > 0 ) && ( checkElemContainment(data,divOffset,'top')  || !isSortableInViewPort(data,scrollDiv,isWindow,height,'top') ) ){
							...
							}else if(scrollDiv === document.documentElement && checkElemContainment(data,divOffset,'top') ){
						...
						else if( isElementReachedBottom( data, divOffset, scrollDivTop, height ) && (scrollTop < _maxScrollHeight) &&  ( checkElemContainment(data,divOffset,'bottom') || !isSortableInViewPort(data,scrollDiv,isWindow,height,'bottom') )  && ( nolastchild || isReachedLastELem( data, height, lastchild, scrollDivTop ) )){
							if(_maxScrollHeight - scrollTop > 10){
								if(isRelativeY && scrollDiv.contains(data._div)){
									data._div.style.top = _mousePosition.y - _offset[1] - parent.top + 10 + scrollTop + "px";
								}else if(scrollDiv === document.documentElement &&  checkElemContainment(data,divOffset,'bottom') ){
			...
			var isSortableInViewPort = function(data,scrollDiv,isWindow,height,side){
				if(data.containment){
					...
					if(side == 'top'){
						...
						return parentOffset.bottom < (top + height - data.scrollBoundaries.bottom);
					}
				}
			}
			var checkElemContainment = function(data,divOffset,side){

				if(data.containment){
					if(side == 'top' ){
						return (data.containmentDimensions.top < divOffset.top);	
					}else if (side == 'bottom'){
						return (data.containmentDimensions.bottom > divOffset.bottom);
					}
