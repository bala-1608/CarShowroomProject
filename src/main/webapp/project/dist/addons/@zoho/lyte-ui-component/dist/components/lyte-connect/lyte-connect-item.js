...
_template:"<template tag-name=\"lyte-connect-item\"> <template is=\"for\" items=\"{{points}}\" item=\"item\" index=\"index\"><template is=\"if\" value=\"{{renderPoint(item,ltPropRenderPoints)}}\"><template case=\"true\"> <span index=\"{{index}}\" class=\"lyteConnectAnchorPoint {{item.class}}\" left=\"{{item.left}}\" top=\"{{item.top}}\" style=\"{{item.style}}\" __mousedown=\"{{action('mousedown')}}\"></span> </template></template></template> <lyte-element-wrapper></lyte-element-wrapper> <template is=\"if\" value=\"{{svgCreate}}\"><template case=\"true\"><div class=\"lyteConnectItemSuggestionWrapper\"> <template is=\"for\" items=\"{{suggestionData}}\" item=\"item\" index=\"index\"> <lyte-suggestion-element onmouseenter=\"{{action('suggest_enter',index)}}\" onmouseleave=\"{{action('suggest_leave',index)}}\" class=\"{{item.class}}\" __click=\"{{action('suggest_click',index)}}\"> <lyte-yield yield-name=\"suggest_icon\"></lyte-yield> </lyte-suggestion-element> </template> </div></template></template> </template>",
