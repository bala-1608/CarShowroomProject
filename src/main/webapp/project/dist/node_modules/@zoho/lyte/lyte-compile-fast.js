...
const htmlToJsObjectLyteRendered = (function () {
    ...
    if (node.nextElementSibling()) {
      ...
      if (type) {
        if (type === "lyte-else") {
          ...
          falseTemp.setAttribute("case", "false")
          if (nextSibling.next && nextSibling.prev && nextSibling && nextSibling.next.nodeValue() && nextSibling.prev.nodeValue() && nextSibling.next.nodeValue().trim() === "" && nextSibling.prev.nodeValue().trim() === "") {
  ...
  const splitTextNodes = (node, checker = {}, errorObj) => {
    ...
    if (node.attr) {
        if(node.hasAttribute("@view-in")){
          node.removeAttribute("@view-in");
          let temp1 = node.createElement("template");
          temp1.setAttribute("lyte-if","{{lyteViewPort()}}");
          temp1.setAttribute("render-view-port-template",node.tag.toLowerCase());
          let temp2 = node.createElement("template");
          temp2.setAttribute("lyte-else","");
          let _node = node.cloneNode();
          _node.child = [];
          _node.appendChildArr(node.child);
          temp2.appendChild(_node);
          let temp3 = node.createElement("template");
          ...
          if(index != undefined){
            ...
            parent.child = [];
            child = child.slice(0,index).concat(temp3.child).concat(child.slice(index+1));
            parent.appendChildArr(child);
            node = parent.querySelector("[render-view-port-template]");
          }
      }
...
Lyte.Compile.$newGetDeepNodes = function (componentName, node, deepNodes, deepN, is, strict, errors, warnings, nearByParent, dontProcessAttr) {
    ...
    if (node.node == "element" && node.tag.indexOf("-") !== -1 && node.tag !== "lyte-yield") {
        ...
        this.$dependentComponents.push(node.tag);
    }
...
Lyte.Compile.$getDynamicNodes =  function(htmlObj,intelligenceObj) {
    ...
    if(htmlObj && htmlObj.nextElementSibling()){
        ...
        while(child){
            ...
            if(child && child.tag == "template" && child.hasAttribute("view-port-template-prerender")){
                viewPortPreRenderTemplate = child;
            }
            child = child.nextElementSibling();
        }
    }
    if (htmlObj && htmlObj.tag === "template" && htmlObj.node == "element") {
        ...
        if (errors.length) {
            ...
            let preRenderTemplate;
            if(viewPortPreRenderTemplate){
                let children = viewPortPreRenderTemplate.children();
                if(children && children.length == 1){
                    let innerContent = viewPortPreRenderTemplate.childNodes();
                    let childElement,flag=true;
                    for(let i = 0; i < innerContent.length; i++){
                        let childNode = innerContent[i];
                        if(childNode.node == "text" && childNode.text && childNode.text.trim()!=""){
                            flag = false;
                        }
                        if(childNode.node == "element"){
                            childElement = childNode;
                        }
                    }
                    if(flag && childElement){
                        viewPortPreRenderTemplate.child  = [];
                        viewPortPreRenderTemplate.appendChild(childElement);
                        viewPortPreRenderTemplate.setAttribute("root","true");
                    }
                }
                preRenderTemplate = viewPortPreRenderTemplate.outerHTML();
                viewPortPreRenderTemplate.remove();
            }
            result = {
                componentName: compName,
...
Lyte.Compile.compileComponent = function(obj, intelligenceObj, options){
    ...
    if ((typeof componentName !== "string" || componentName == "") && !templateNamePrefix) {
        ...
        if (obj.querySelector("template")) {
            ...
            let dpComps = [];
            if(options){
                if(options.dependentComponents){
                    dpComps = options.dependentComponents;
                }else{
                    options.dependentComponents = dpComps;
                }
            }
            if(returnedObj.dependentComponents){
                returnedObj.dependentComponents.forEach(function(compName){
                    if(!compName.match(/^(lyte-|crux-)/g)){
                        dpComps.push(compName);
                    }
                })
            }
            ...
            let errObb = { errors : []};
            if(observedAttributes && observedAttributes.length && intelligenceObj && intelligenceObj.directives && intelligenceObj.directives[componentName] && (intelligenceObj.directives[componentName].data || intelligenceObj.directives[componentName].prop) ){
                let dataAndProp = [intelligenceObj.directives[componentName].data, intelligenceObj.directives[componentName].prop];
                dataAndProp.forEach(function(dp){
                    if(dp && dp.length){
                        dp.forEach(function(obj){
                            if(observedAttributes.indexOf(obj.propName) != -1){
                                errObb.errors.push({ "message": "PropName found in data. - " + componentName });
                            }
                        })
                    }
