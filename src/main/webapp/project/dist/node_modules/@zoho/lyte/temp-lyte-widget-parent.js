...
window.DataBindingLayer = {
    ...
    "sendDataUpdateToFrame" : function(element, keyValuePairs) {
        ...
        function origSendDataUpdate() {
            ...
            if(shadow == actualElement){ // element nt in dmm
				shadow = actualElement._target.getRootNode();
			}
...
;(function(Lyte) {
        ...
        if(data.c) {
            ...
            switch(data.c) {
                ...
                case "getDetails" : {
                    ...
                    switch(actData._m) {
                        case "window" : {
                            if(diffProps.window[actData.prop] && window.widgetPreviewNode) {
                                actData.prop = diffProps.window[actData.prop];
                                target = window.widgetPreviewNode;
                            } else {
                                target = window;
                            }
                        }
                        ...
                        case "documentElement" : {
                            if(diffProps.documentElement[actData.prop] && window.widgetPreviewNode) {
                                actData.prop = diffProps.documentElement[actData.prop];
                                target = window.widgetPreviewNode;
                            } else {
                    ...
                    if(actData.isProp) {
                        ...
                        if(window.widgetPreviewNode) {
                            if(actData.prop == "getBoundingClientRect") {
                                let previewNodeRect = window.widgetPreviewNode.getBoundingClientRect();
                                let actRet = {};
                                let xPos = previewNodeRect.x;
                                let yPos = previewNodeRect.y;
                                actRet.x = prop.x - xPos;
                                actRet.y = prop.y - yPos;
                                actRet.left = prop.left - xPos;
                                actRet.right = prop.right - xPos;
                                actRet.top = prop.top - yPos;
                                actRet.bottom = prop.bottom - yPos;
                                actRet.height = prop.height;
                                actRet.width = prop.width;
                                prop = actRet;
                            } 
                            // else if(actData.prop == "getComputedStyle") {
                                    ...
                                    switch(ee.type) {
                                        ...
                                        case "element" : {
                                            ...
                                            if(ee.tagName == "svg") {
                                                ...
                                                element._target = ee.prev ? mappings[ee.prev].parentNode : target;
                                            } else {
                                                ...
                                                if(isParentComp ) {
                                                    ...
                                                    element._m = widgetObj.count++;
                                                    element._target = ee.prev ? mappings[ee.prev].parentNode : target;
                                                    element.setAttribute("parent-wrapper-comp", true);
                                                    element.attachShadow({"mode" : "open"});
                                                    docEventListenerHandler.attachShadow(element.shadowRoot, widgetObj.shadow);
                                                    ...
                                                    try {
                                                        docEventListenerHandler.copyStyles(widgetObj.shadow, element.shadowRoot);
                                                    } catch(e) {
                                                        console.log("Error during copyStyles")
                                                    }
                                            ...
                                            let initMethods = ee.initMethods;
                                            for(var key in ee.attrs) {
                                                if(key == "node-id") {
                                                    ...
                                                    let _url;
                                                    if(ee.attrs[key].startsWith("blob:") || ee.attrs[key].startsWith("mailto:") || ee.attrs[key].startsWith("http://") || ee.attrs[key].startsWith("https://") || ee.attrs[key].startsWith("#")) {
                                                        _url = ee.attrs[key] 
                                                    } else {
                                                ...
                                                } else if(initMethods && initMethods.indexOf(key) != -1 && isParentComp) {
                                                    let x = data.x;
                                                    // var methodName = key;
                                                    let methodCamelCase = _LC.String.toCamelCase(key);
                                                    let elm1 = element;
                                                    element.setMethods(methodCamelCase, function() { //eslint-disable-line no-loop-func
                                                        ...
                                                        args.forEach(function(item) {//eslint-disable-line no-loop-func
                                                            if(item instanceof Event) {
                                                                sendArgs.push(getDummyEvent(item));
                                                            } else if(item instanceof Node) {
                                                                let obj = {};
                                                                ["offsetTop", "offsetLeft", "offsetWidth", "offsetHeight", "localName", "tagName" , "nodeName", "nodeType", "textContent"].forEach(function(keyName) {
                    ...
                    if(!eventTypeObj) {
                        eventTypeObj = document.parentListeners[data.y] = {
                            "mainFunction" : function(event) {
                                ...
                                var eventObj = {};
                                for(var key in event) {
                                    if(!(event[key] instanceof HTMLElement) && !(event[key] instanceof SVGElement) && typeof event[key] !== "function" && !(event[key] instanceof Window) && !(event[key] instanceof Document) && !(event[key] instanceof Text) && key !== "sourceCapabilities" ) {
                                        eventObj[key] = event[key];
                                    }
                                }
    ...
    function registerEventsForShadow(self, shadow, flag, subwidgetObj) {
        ...
        try {
            docEventListenerHandler.attachShadowGlobal(shadow);
        } catch(e) {
            console.log("Error in docEventListener global listener addition");
        }
        ...
        shadow._addEventListener = function(target, type, id) {
            shadow._eL[id] = function(event) {
                ...
                childWin.postMessage({"p" : "3" , "x" : self.id, "1" : target1._m, "2" : type, "3" : target1.value, "4" : eventObj},"*");
            }; 
        ...
        shadow.addEventListener("click", function(event) {
            if(event.target.tagName == "INPUT" && event.target.getAttribute("type") == "file") {
                ...
                if((window.clean || true) && !event._doneAlready) {
                    event._doneAlready = true;
                    var eventObj = {"button" : event.button};
        ...
        for(var i=0;i<arr.length;i++) { //eslint-disable-line no-loop-func
            shadow.addEventListener(arr[i], function(event) { //eslint-disable-line no-loop-func
                ...
                let eventObj = getDummyEvent(event);
    ...
    function handleInitPropertiesParent(element,ee) {
        ...
        element.setData(ee.initProperties,{postmessage : false});
    }
    /*disable async function*/
    function iterateNodesParent(element, childNodes, widgetObj, isSvgParent) {
        let nodeValueCopies = ["offsetTop", "offsetLeft", "offsetWidth", "offsetHeight", "localName", "tagName" , "nodeName", "nodeType", "textContent"];
        var mappings = widgetObj.mappings;
        childNodes.forEach(function(item) {
            let initMethods = item.initMethods;
            if(item.type == "element") {
                ...
                if(item.tagName == "svg" || isSvgParent) {
                    ...
                    if(isParentComp) {
                        ...
                        newElement.setAttribute("parent-wrapper-comp", true);
                        newElement.attachShadow({"mode" : "open"});
                        docEventListenerHandler.attachShadow(newElement.shadowRoot, widgetObj.shadow);
                        ...
                        shadowComp.setAttribute("comp-in-parent", "");
                        try {
                            docEventListenerHandler.copyStyles(widgetObj.shadow, newElement.shadowRoot)
                        } catch(e) {
                            console.log("Error during copyStyles");
                        }
                        ...
                        switch(key) {
                            ...
                            default : { 
                                if(isSvgParent) {
                                    ...
                                    if(initMethods && initMethods.indexOf(key) != -1 && isParentComp) {
                                        ...
                                        newElement.setMethods(methodCamelCase, function() {
                                            ...
                                            args.forEach(function(item) {//eslint-disable-line no-loop-func
                                                if(item instanceof Event)  {
                                                    ...
                                                    let obj = {};
                                                    nodeValueCopies.forEach(function(keyName) {
                                                        obj[keyName] = item[keyName];
                                                    })
                                                    sendArgs.push(obj);
                                                } else {
    ...
    window.Lyte.Component.register("lyte-widget", {
            ...
            didConnect : function() {	
                ...
                // docEventListenerHandler.docShadows.push(this.childDiv.shadowRoot); // bug fix slider component issue
                // _widgetLyte.__lc.shadow.attachEvents(this.childDiv.shadowRoot); // bug fix slider component issue
    ...
    if(resourcesInParent && resourcesInParent.length) {
        ...
        resourcesInParent.forEach(function(item) {
            ...
            if(type == "script") {
                ...
                Lyte.__lc.styleSheet.preload(url)
                styles.push(url);
            }
        });
...
(function(Lyte) {
    ...
    Lyte.Widget.addComponent = function(widgetName, resources, componentName) {
        var fObj = {
            "p" : "1a", 
            "w" : widgetName, 
            "c" : componentName,
        ...
        try {
            if(resources && Array.isArray(resources)) {
                resources.forEach(function(item) {
                    if(item.type == "style" && !item.styleTag) {
                        Lyte.__lc.styleSheet.preload(item.url);    
                    }
                });
            }
        } catch(e) {
    ...
    Lyte.__widgetNamespaces = {};
   
    window.LyteWidgetNamespace = class LyteWidgetNamespace {

        static _globalEventHandler = {
            "listeners" : {}
        }
                ...
                if(!triggerNamespaces.length) {
                    ...
                    listenerArr.forEach(function(listener) {
                        try {
                            ...
                            if(execute) {
                                listener.func.call(window, data);
                            }
                        } catch(e) {
        ...
        constructor(name) {
            ...
            this.eventCounter = 0;
            Lyte.__widgetNamespaces[name] = this;
        }
    
        triggerEvent(eventName, data, options, wName) {
            // let widgetName = wName || this._ins.__wName;
            let widgetName = this.__name;
            if(!options || !options.target) {
                options = { "target" : [widgetName]}
            } else if(typeof options.target != "string") {
                options.components = {};
                options.target.forEach(function(item, index) {
                    if(typeof item != "string") {
                        let onlyKey = Object.keys(item)[0];
                        options.components[onlyKey] = item[onlyKey];
                        options.target[index] = onlyKey;
                    }
                    if(options.target[index] == "_self") {
                        options.target[index] = widgetName;
                    }
                });
            }
            LyteWidgetNamespace.triggerEvent(eventName, data, options, widgetName || this.__name);
        }
    
        removeEventListener(listenerId ) {
            let widgetName = this.__name;
            LyteWidgetNamespace.removeEventListener(listenerId, widgetName);
        }
    
        addEventListener(eventName, func, options, wName) {
            // let widgetName = wName || this._ins.__wName;
            if(eventName.indexOf("-") != -1) {
                console.error("Event Name can't be hypenated");
            }
            ...
            LyteWidgetNamespace.addEventListener(eventName, func, options, widgetName || this.__name, listenerId);
            return listenerId;
        }
    }

    Lyte.Widget.registerNamespace = function(name) {
        if(Lyte.__widgetNamespaces[name]) {
            console.error("Namespace " + name + " has already been registered");
        }
        return new WidgetNamespace(name);
    }
