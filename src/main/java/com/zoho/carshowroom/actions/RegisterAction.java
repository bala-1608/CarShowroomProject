...
import com.zoho.carshowroom.engines.RegisterEngine;
...
public class RegisterAction extends ActionSupport {
	...
	private ZAddress address = new ZAddress();
	private String httpMethod;
	private Map<String, Object> jsonResponse;
	private RegisterEngine engine = new RegisterEngine();
	...
	public void validate() {
		...
		if (user.getRole() == null) {
			...
			try {
				...
				if (role < 0 || role > 3) {
					addFieldError("role",
							"Role must be 0 (User), 1 (Showroom Manager), 2 (Company Admin), or 3 (Application Admin)");
				}
			} catch (IllegalArgumentException e) {
	...
	public String execute() {
		...
		case "post":
				...
				try {
					...
					jsonResponse = engine.post(user, address);
